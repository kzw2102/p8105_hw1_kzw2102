...
```
In this data set, there are `r nrow(x)` rows (or number of subjects) and `r ncol(y)` columns, each denoting a different kind of variable. The mean of of our sample is `r mean(x) `, the median is ` r median(x)`, and the standard deviation is ` r sd(x)`.The proportion of cases for which x+y>1 is `length(which (vec_logical="True")/sum(x,y)`.
